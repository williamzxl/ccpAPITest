...
func TestP1(t *testing.T) { TestingT(t) } //继承testing的方法，可以直接使用go test命令运行

type MySuiteP1 struct{} //创建测试套件结构体

var _ = Suite(&MySuiteP1{})

func (s *MySuiteP1) TestUserAuthP1Success(c *C) {
...
func (s *MySuiteP1) TestUserAuthP1Fail(c *C) {
	logger.Info("this is test")
	conn, err := core.DialTimeout("139.199.128.158:8085", 2*time.Second)
	if err != nil {
		logger.Infof("make session erros: %s", err)
